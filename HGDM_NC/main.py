...
from Model import Public_diffusion_model
...
class Coach:
    ...
    def run(self):
        for ratio in range(len(self.handler.train_idx)):
            ...
            print("\t[Classification] Macro-F1: {:.4f} var: {:.4f}  Micro-F1_mean: {:.4f} var: {:.4f} auc {:.4f}"
    ...
    def prepareModel(self):
        ...
        self.model = Public_diffusion_model(self.dim).to(device)
    ...
    def test(self):
        ...
        args.load_model = '/'+'retain_100w_lr_0.01_reg_1e-06_batch_4096_noise_scale_0.0005_step_100.mod'
        self.loadModel()
        
        reses = self.testEpoch_retain()
