...
class Retain_diffusion_Model(nn.Module):
    def __init__(self,u_feat,t_u_feat):
        super(Retain_diffusion_Model, self).__init__()
        
        self.u_feat = u_feat
        self.t_u_feat = t_u_feat
        ...
        self.i_feat = nn.Parameter(init(torch.empty(args.item, args.latdim)))
        self.task_i_feat = nn.Parameter(init(torch.empty(args.task_item, args.latdim)))
        self.t_i_feat = nn.Parameter(init(torch.empty(args.t_item, args.latdim)))
        self.t_task_i_feat = nn.Parameter(init(torch.empty(args.t_task_item, args.latdim)))
        ...
        for i in range(0, args.gcn_layer):
            self.layers.append(DGLLayer(args.latdim, args.latdim, weight=self.weight, bias=False, activation=self.act))
        for i in range(0, args.gcn_layer):
            self.layers2.append(DGLLayer(args.latdim, args.latdim, weight=self.weight, bias=False, activation=self.act))
        for i in range(0, args.uugcn_layer):
            self.uu_Layers.append(UUGCNLayer(args.latdim, args.latdim, weight=self.weight, bias=False, activation=self.act))
        
        self.transform_layer = torch.nn.Linear(self.u_feat.shape[1],args.latdim)
        self.dense = torch.nn.Linear(args.latdim,1)
    ...
    def forward(self, adj, ui_adj,task_ui_adj, is_training=True):

        if is_training==True:
            self.uEmbeds = self.transform_layer(self.u_feat)
            self.iEmbeds = self.i_feat
            self.taskiEmbeds = self.task_i_feat
        else:
            self.uEmbeds = self.transform_layer(self.t_u_feat)
            self.iEmbeds = self.t_i_feat
            self.taskiEmbeds = self.t_task_i_feat
        uu_embedding = [self.uEmbeds]
        for i, layer in enumerate(self.uu_Layers):
            embeddings = layer(adj, uu_embedding[-1])
            norm_embeddings = F.normalize(embeddings, p=2, dim=1)
            uu_embedding += [norm_embeddings]

        uu_embedding = sum(uu_embedding)


        embeds = torch.concat([self.uEmbeds, self.iEmbeds], axis=0)
        taskembeds = torch.concat([self.uEmbeds, self.taskiEmbeds], axis=0)
        all_embeddings1 = [embeds]
        all_embeddings2= [taskembeds]

        for i, layer in enumerate(self.layers):
            embeddings = layer(ui_adj, all_embeddings1[-1][:self.uEmbeds.shape[0]], all_embeddings1[-1][self.uEmbeds.shape[0]:])
            norm_embeddings = F.normalize(embeddings, p=2, dim=1)
            all_embeddings1 += [norm_embeddings]
        all_embeddings1 = sum(all_embeddings1)

        for j, layer in enumerate(self.layers2):
            embeddings = layer(task_ui_adj, all_embeddings2[-1][:self.uEmbeds.shape[0]], all_embeddings2[-1][self.uEmbeds.shape[0]:])
            norm_embeddings = F.normalize(embeddings, p=2, dim=1)
            all_embeddings2 += [norm_embeddings]
        all_embeddings2 = sum(all_embeddings2)
        # gcn_u_embeds = all_embeddings[:self.uEmbeds.shape[0]]

        # total_ulats = ulats+gcn_u_embeds
        # total_ulats = self.pooling(total_ulats)
        # scores = self.dense(total_ulats)
        # scores = self.softmax(scores)

        return all_embeddings1,all_embeddings2,uu_embedding
    
    

    def cal_loss(self, ancs, label,uadj,uiadj,taskuiadj):
        gcnEmbeds1,gcnEmbeds2,uuEmbeds = self.forward(uadj,uiadj,taskuiadj)
        gcn_u_Embeds1 = gcnEmbeds1[:self.uEmbeds.shape[0]]
        gcn_u_Embeds2 = gcnEmbeds2[:self.uEmbeds.shape[0]]
        gcn_u_Embeds = gcn_u_Embeds1+gcn_u_Embeds2

        u_diff_loss,diff_usrEmbeds= self.diffusion_model.training_losses2(self.user_denoise_model, uuEmbeds, gcn_u_Embeds, ancs)
        diff_loss = u_diff_loss.mean()
        all_u_embeddings = gcn_u_Embeds+diff_usrEmbeds
        scores = self.dense(all_u_embeddings)
        scores = self.softmax(scores)

        batch_u = scores[ancs].reshape(-1)
        batch_label = label
        bce_loss = self.criterion(batch_u,batch_label)
        
        #reg_loss
        reg_loss = self.reg_params(self) * args.reg
        #cl loss
        # cl_loss = 0
        # for i in range(args.uugcn_layer):
        #     embeds1 = gcn_embeds_list[i].detach()
        #     embeds2 = hyper_embeds_list[i]
        #     cl_loss += self.cal_infonce_loss_spec_nodes(embeds1, embeds2, t.unique(ancs),args.temp)
        # cl_loss *= args.ssl_reg
        return bce_loss,reg_loss,diff_loss
    def cal_auc(self,uadj,uiadj,taskuiadj):
        gcnEmbeds1,gcnEmbeds2,uuEmbeds = self.forward(uadj,uiadj,taskuiadj,False)
        gcn_u_Embeds1 = gcnEmbeds1[:self.uEmbeds.shape[0]]
        gcn_u_Embeds2 = gcnEmbeds2[:self.uEmbeds.shape[0]]
        gcn_u_Embeds = gcn_u_Embeds1+gcn_u_Embeds2
        diff_usrEmbeds= self.diffusion_model.p_sample(self.user_denoise_model, gcn_u_Embeds, args.sampling_steps)
        all_u_embeddings = gcn_u_Embeds+diff_usrEmbeds
        scores = self.dense(all_u_embeddings)
        scores = self.softmax(scores)
    ...
    def cal_infonce_loss_spec_nodes(self,embeds1, embeds2, nodes, temp):
        ...
        nume = torch.exp(t.sum(pckEmbeds1 * pckEmbeds2, dim=-1) / temp)
...
class Retain_gcn_Model(nn.Module):
    def __init__(self,u_feat,t_u_feat):
        super(Retain_gcn_Model, self).__init__()
    ...
    def cal_loss(self, ancs, label,uadj,uiadj,taskuiadj):
        ...
        all_u_embeddings = gcn_u_Embeds1+gcn_u_Embeds2+uuEmbeds
        scores = self.dense(all_u_embeddings)
        scores = self.softmax(scores)
        ...
        batch_label = label
        bce_loss = self.criterion(batch_u,batch_label)
        y_ture = batch_label.detach().cpu().numpy()
        y_pred =  batch_u.detach().cpu().numpy()
        auc_score = roc_auc_score(y_ture, y_pred)

        #reg_loss
        reg_loss = self.reg_params(self) * args.reg
        return bce_loss,reg_loss, auc_score
    def cal_auc(self,uadj,uiadj,taskuiadj):
        gcnEmbeds1,gcnEmbeds2,uuEmbeds = self.forward(uadj,uiadj,taskuiadj,False)
        gcn_u_Embeds1 = gcnEmbeds1[:self.uEmbeds.shape[0]]
        gcn_u_Embeds2 = gcnEmbeds2[:self.uEmbeds.shape[0]]
...
class Public_diffusion_model(nn.Module):
    def __init__(self,f_dim):
        super(Public_diffusion_model, self).__init__()
        out_dims = eval(args.dims) + [args.latdim]
        in_dims = out_dims[::-1]
        self.user_denoise_model = Denoise(in_dims, out_dims, args.d_emb_size, norm=args.norm)
        self.diffusion_model = GaussianDiffusion(args.noise_scale, args.noise_min, args.noise_max, args.steps)
